...
public class LightShooterBehavior : MonoBehaviour
{
   ...
   [SerializeField] private ParticleSystem explosionEffect;
    ...
    public float Exp = 34f;
    public float gold = 25f;
    ...
    IEnumerator ShootingSequence()
    {
        while (true)
        {
            // Shoot 1 projectile
            ShootProjectiles(1);
            yield return new WaitForSeconds(timeBetweenShots);  // Wait 2 seconds
            ...
            // Shoot 3 projectiles
            ShootProjectiles(3);
            yield return new WaitForSeconds(3f);  // Wait 4 seconds
        }
    }

    void ShootProjectiles(int count)
    {
        StartCoroutine(ShootSequence(count));
    }

    IEnumerator ShootSequence(int count)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            // Wait for a short period between shots (0.5 seconds)
